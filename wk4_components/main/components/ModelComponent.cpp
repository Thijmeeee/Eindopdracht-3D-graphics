	...
	for (auto group : groups)
	{
		...
		for (auto face : group->faces)
		{
			for (Vertex vertex : face.vertices)
			{
				if (vertex.normal >= 0)
				{
					group->model.push_back(tigl::Vertex::PTN(vertices[vertex.position], texcoords[vertex.texcoord],
						normals[vertex.normal]));
				}
				else
				{
					group->model.push_back(tigl::Vertex::PT(vertices[vertex.position], texcoords[vertex.texcoord]));
				}
			}
		}
	}
